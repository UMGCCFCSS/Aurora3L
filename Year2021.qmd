...
The2021 <- HolisticToArchived(data=The2021, manufacturer="Cytek",
 baselinecutoffs=QC_Cutoffs, gainmultiplier = 2)
```

## {.sidebar}
Dashboard contains historical data from **2021** for the **Cytek Aurora 3L**.
...
CSVname <- paste0("Historical_Aurora3L_2021.csv")
SaveHere <- file.path(MainFolder, CSVname)
write.csv(The2021, SaveHere, row.names=FALSE)
```
